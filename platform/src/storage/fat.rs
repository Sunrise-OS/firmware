...
impl Read for Adapter {
    ...
    fn read(&mut self, buffer: &mut [u8]) -> hadris_io::Result<usize, Self::Error> {
        ...
        while done < buffer.len() {
            ...
            // Whole sectors go straight into the caller's buffer in one request
            // (up to 1 MiB): a file read is then a handful of transfers rather
            // than one per sector.
            let whole = ((buffer.len() - done) as u64 / block)
                .min(self.device.block_count() - lba)
                .min(2048);
            if within == 0 && whole > 0 {
                let bytes = (whole * block) as usize;
                self.device
                    .read(lba, &mut buffer[done..done + bytes])
...
pub fn mount(device: Arc<dyn BlockDevice>) -> Option<Arc<dyn FileSystem>> {
    let volume = match FatVolumeBuilder::new(Adapter::new(device)).open() {
        Ok(volume) => volume,
        Err(error) => {
            log::info!("fat: not a mountable volume: {error:?}");
            return None;
        }
    };
    let fat_type = volume.fat_type();
    log::info!("fat: mounted a {fat_type} volume");
    Some(Arc::new(Volume {
        inner: Arc::new(Inner {
...
impl FileSystem for Volume {
    ...
    fn open(&self, path: &str) -> Option<Box<dyn FileNode>> {
        Some(Box::new(lookup(&self.inner, path)?))
    }
}
